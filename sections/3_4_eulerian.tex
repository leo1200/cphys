...
\textcolor{blue1}{Step 1}: Integrate the conservation law over a cell and time step and recognize the average
defined in Eq. \ref{eq:cell_avg}.
...
\begin{equation}
    \begin{gathered}
        ...
        = \int_{x_{i-\frac{1}{2}}}^{x_{i+\frac{1}{2}}}\left[\vec{U}\left(x, t_{n+1}\right)-\vec{U}\left(x, t_n\right)\right] d x+\int_{t_n}^{t_{n+1}}\left[\vec{F}\left(x_{i+\frac{1}{2}}, t\right)-\vec{F}\left(x_{i-\frac{1}{2}}, t\right)\right] dt \\
        \underbrace{=}_{\text{recognize average}} \Delta x\left[\vec{U}_i^{(n+1)}-\vec{U}_i^{(n)}\right]+\int_{t_n}^{t_{n+1}}\left[\vec{F}\left(x_{i+\frac{1}{2}}, t\right)-\vec{F}\left(x_{i-\frac{1}{2}}, t\right)\right] d t=0
    \end{gathered}
    \label{eq:finite_volume_scheme_step1}
\end{equation}

\textcolor{blue1}{Step 2}: \textcolor{blue1}{Between any two cells we essentially have a Riemann problem whose solution determines the flux between the cells}.
\begin{equation}
...
\end{equation}
\note{The Euler equations have no intrinsic scale, with a step function as input (also without intrinsic scale), the solution
can only depend on 
\begin{equation}
    \xi = \frac{x - x_{i + \frac{1}{2}}}{t-t_n}
\end{equation}
such that $\vec{U}(x,t) = \vec{U}(\xi)$ and for $x = x_{i+\frac{1}{2}}$ where $\xi = 0$,
$\vec{U}$ is constant such that the flux
\begin{equation}
    \vec{F}(x_{i+\frac{1}{2}},t) = \vec{F}^*_{i+\frac{1}{2}} = \vec{F}_{\text{Riemann}}(\vec{U}_i^{(n)}, \vec{U}_{i+1}^{(n)})
\end{equation}
too is constant (as long as $x_{i+\frac{1}{2}}$ is not reached by information from adjacent interfaces) \citep[Sec. 6.4.2]{dullemond}.
}
\begin{mdframed}[style=padded]
    ...
    \end{equation}
    This defines an update scheme, the first order Godunov scheme, for the fluid variables on the cells (with some appropriate initial and boundary conditions). $\vec{F}^*_{i-\frac{1}{2}}$ and $\vec{F}^*_{i+\frac{1}{2}}$ are black-boxes for now - to find the fluxes we need to solve the Riemann problem, which we will later approximate, for example, with the HLL solver.
\end{mdframed}
...
\begin{equation}
    \begin{gathered}
        f^{H L L}=\frac{S_R f_L-S_L f_R+S_L S_R\left(u_R-u_L\right)}{S_R-S_L} \\
