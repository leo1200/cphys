...
\begin{equation}
    \frac{1}{K} \partial_t \vec{x} = L \vec{x} - \vec{b}, \quad \text{for } \partial_t \vec{x} = 0 \text{ for } t \rightarrow \infty, \quad \text{some factor } K { in } \frac{\m^2}{s}
\end{equation}
...
\begin{equation}
    ...
    \phi_{i, j}^{(n+1)}=\frac{K \Delta t}{\Delta x^2}\left(\phi_{i, j}^{(n+1)}+\phi_{i-1, j}^{(n)}+\phi_{i, j+1}^{(n)}+\phi_{i, j-1}^{(n)}\right)+\underbrace{\left(1-4 \frac{K \Delta t}{\Delta x^2}\right)}_{=0 \text{by choice } \Delta t = \frac{1}{4} \frac{K}{\Delta x^2}} \phi_{i, j}^{(n)}-4 \pi G \rho_{i j} \Delta t
\end{equation}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.9\textwidth]{figures/vcycle.svg}
    \caption{V-cycle.}
    \label{fig:vcycle}
\end{figure}
