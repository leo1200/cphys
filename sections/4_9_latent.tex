...
\begin{itemize}
    \item \textbf{Probabilistic Encoder:} The encoder $h_{\vec{\Omega}_{\text{Enc}}}(\vec{x})$ takes input points from the data space $\vec{x}$ and outputs
    ...
    parameters $\vec{\theta}$ for the likelihood $p_{\vec{\theta}}(\vec{x}|\vec{z})$, from 
    which we can sample an $\vec{x}$. (Only mapping to the mean of the likelihood is also possible.)
\end{itemize}

\greenbox{While the distributions $q_{\vec{\phi}(\vec{x})}(\vec{z})$ and $p_{\vec{\theta}}(\vec{x}|\vec{z})$ might generally
be very simple, as of the possibly complex mappings $h_{\vec{\Omega}_{\text{Enc}}}$ and $f_{\vec{\Omega}_{\text{Dec}}}$, the
...
\subsubsubsection{Estimation of the ELBO}
...
${\vec{\phi}} = h_{\vec{\Omega}_{\text{Enc}}}(\vec{x})$ and draw
\begin{equation}
...
\begin{equation}
    \text{ELBO}_{\vec{\theta}, \vec{\phi}}(\vec{x}) \approx \frac{1}{L} \sum_{l=1}^{L} \left(\log p_{\vec{\theta}}(\vec{x}|\vec{z}^{(l)}) + \log p(\vec{z}^{(l)}) - \log q_{\vec{\phi}(\vec{x})}(\vec{z}^{(l)})\right)
\end{equation}

\subsubsubsection{ELBO-gradients}

\bluebox{For being able to backpropagate to $\vec{\Omega}_{\text{Enc}}$ and $\vec{\Omega}_{\text{Dec}}$, we need to calculate the gradient of the ELBO with respect to the parameters
\begin{equation}
...
\end{equation}
we cannot put the derivative inside the expectation, because this expectation (how we sample the $\vec{z}$) depends on the parameters $\vec{\phi}$ itself. Which samples we draw to 
approximate the ELBO depends on the parameters $\vec{\phi}$.
}
...
\bluebox{\textbf{Generative process and a note on the prior:} For generative modeling
we sample $\vec{z} \sim p(\vec{z})$, obtain parameters $\vec{\theta}$ of the likelihood from
which we can then sample $\vec{x} \sim p_{\vec{\theta}}(\vec{x}|\vec{z})$. But what is the prior $p(\vec{z})$?

In a vanilla VAE the prior $p(\vec{z})$ is set and fixed (typically standard normal). However, the prior 
enters the ELBO gradient $\partial_\vec{\phi} \operatorname{ELBO}$ as sampling of $\vec{z}$
depends on $\vec{\phi}$ (this becomes obvious when applying the reparameterization trick).

In more advanced setups
another model is trained to obtain a learned prior $p(\vec{z})$ (as for instance in the original DALLE
\footnote{
