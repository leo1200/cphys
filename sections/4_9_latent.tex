...
We have
\begin{equation}
    \log p_\vec{\theta}(\vec{x}) = \log \int_{\vec{z}} q_\vec{\phi}(\vec{z}) \frac{p_\vec{\theta}(\vec{x},\vec{z})}{q_\vec{\phi}(\vec{z})} \, d\vec{z} \underset{\text{Jenssen's inequality}}{\geq} \int_{\vec{z}} q_\vec{\phi}(\vec{z}) \log \frac{p_\vec{\theta}(\vec{x},\vec{z})}{q_\vec{\phi}(\vec{z})} \, d\vec{z}
\end{equation}
where Jenssens inequality holds because $\log$ is concave.

\pinkbox{We see that if $q_{\vec{\phi}}(\vec{z}) = p_{\vec{\theta}}(\vec{z} | \vec{x})$, then the ELBO is equal to the log-likelihood, as by the \textit{path-rule}
\begin{equation}
...
\idea{Approximate the posterior $p(\vec{z}|\vec{x})$ by $q(\vec{z}) \in \mathcal{Q}$}
\begin{equation}
    \begin{aligned}
        q^*(\vec{z}) &= \argmin_{q(\vec{z}) \in \mathcal{Q}} D_{KL}(q(\vec{z})||p(\vec{z}|\vec{x})) \\
...
\begin{equation}
    q_{\vec{\phi}}(\vec{z}|\vec{x}) \underset{\text{goal}}{\approx} p_{\vec{\theta}}(\vec{z}|\vec{x})
\end{equation}
Finding a good approximation $q_{\vec{\phi}}(\vec{z}|\vec{x})$ later amortizes, because we can then
...
previously. The distributions $q_\vec{\phi}(\vec{z})$ are normally parameterized for each individual data point in a separate optimization process.
More explicitly, one would always write $q_{\vec{\phi}}(\vec{z}|\vec{x})$.}
...
\greenbox{While the distributions $q_{\vec{\phi}}(\vec{z})$ and $p_{\vec{\theta}}(\vec{x}|\vec{z})$ might generally
...
\begin{equation}
    \partial_\vec{\phi} \text{ELBO}_{\vec{\theta}, \vec{\phi}}(\vec{x}) = \partial_\vec{\phi} E_{\vec{z} \sim q_{\vec{\phi}}(\vec{z})} \left[ \log \frac{p_\vec{\theta}(\vec{x},\vec{z})}{q_{\vec{\phi}}(\vec{z})} \right]
\end{equation}
...
\greenbox{\textbf{Reparametrization trick}: Write $z^{(l)}$ as a deterministic function depending on $\vec{\phi}$ and a parameter-free random variable $\epsilon^{(l)}$ which is independent of $\vec{\phi}$.

\begin{equation}
    z^{(l)} = g_{\vec{\phi}}(\epsilon^{(l)}, \vec{x})
\end{equation}

for instance assume

\begin{equation}
    z^{(l)} \sim \mathcal{N}(\mu_{\vec{\phi}}(\vec{x}), \sigma_{\vec{\phi}}(\vec{x})) \quad \rightarrow \quad z^{(l)} = \mu_{\vec{\phi}}(\vec{x}) + \sigma_{\vec{\phi}}(\vec{x}) \epsilon^{(l)}, \quad \epsilon^{(l)} \sim \mathcal{N}(0,1)
\end{equation}

(where $\mu$ and $\sigma$ are the outputs of the encoder) then we can rewrite

\begin{equation}
    E_z[h(z)] = E_{\epsilon} \left[ h(g_{\vec{\phi}}(\epsilon, \vec{x})) \right]
\end{equation}
}

So we do not directly sample from $q_{\vec{\phi}}(\vec{z})$, but from a surrogate model, where the distribution does not
