...
\begin{equation}
    L\hat{\phi} + \hat{s} = 0, \quad \text{ linear differential operator } \hat{L}, \text{ source term } \hat{s}, \text{ solution } \hat{\phi}
\end{equation}
...
\yellowbox{The general Galerkin principle is to multiple an equation by arbitrary test functions and describe the unknown field with the same set of basis functions.}
...
\begin{equation}
    \int_{x_L}^{x_R} S_i(x) \partial_x^2 \phi(x) d x=-\int_{x_L}^{x_R} \partial_x S_i(x) \partial_x \phi(x) d x=-\int_{x_L}^{x_R} S_i(x) 4 \pi G \rho(x) d x=b_i
\end{equation}
so the with the RHS
\begin{equation}
