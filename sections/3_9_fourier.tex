...
\bluebox{For two vectors $\vec{f}$ of length $N$ and $\vec{g}$ of length $M$ one can append zeros to both so they are 
both of length $N+M-1$ (the length of the linear convolution). The cyclic convolution of these extended signals
matches the linear convolution of the original vectors.}

However, this cannot be applied to our convolution of interest 

\begin{equation}
    \Phi(\vec{x}) = g \star \rho
\end{equation}

directly because the Green's function has infinite support, extending the potential
with zeros would cut-off the gravitational interaction.

\greenbox{In the Hockney-Eastwood method \citep{eastwood79} we zero-pad the density array $[\rho_0,\cdots, \rho_{N-1}] \in \mathbb{R}^N$
to $[\rho_0,\cdots, \rho_{N-1},0,...,0] \in \mathbb{R}^{2N}$ but extend the Green's function such that
the cyclic convolution reflects isolated-boundary conditions, by evaluating the Green's function at

\begin{equation}
    [0, 1, 2, ..., N-1, N, ..., 1] \cdot \Delta x
\end{equation}

An intuition for the Hockney-Eastwood method is provided
in Fig. \ref{fig:hockney}, for the details of this technique, we refer to \cite{eastwood79}.}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=1.0\textwidth]{figures/hockney_easwood.pdf}\hfill
  \caption{Illustration of the Hockney-Eastwood in one
  dimension.}
  \label{fig:hockney}
\end{figure}

\paragraph{Beyond the Hockney-Eastwood method} James's method \citep{james77} improves on the computational cost $\mathcal{O}((2N)^d \log 2N)$ of the Hockney-Eastwood method
based on finding a set of correction charges on the boundary (and convolving with a suitable Green's function) - mesh-doubling in 
James's method is only necessary for boundary points. The Vico-Greengard-Ferrando method features improved 
accuracy at larger grid-spacing \citep{zou21}.
