...
\greenbox{$F_s(\vec{r}_i)$ is defined everywhere and differentiable as the kernel is differentiable.}
...
\note{The support size of SPH-particles is usually chosen to be $2h$ so that $W(||\vec{r}_i - \vec{r}'||, h) = 0$ for $||\vec{r}_i - \vec{r}'|| > 2h$, see figure \ref{fig:sph}.}
...
length into the calculation of our fluid quantities - the
...
\begin{figure}[!htb]
 \centering
 \includesvg[width=0.8\textwidth]{figures/sph2.svg}\hfill
...
\idea{Such shear flows are marked by high vorticity $\vec{\omega} = \vec{\nabla} \times \vec{v}$, so for high vorticity (high rotation of the velocity field) compared to the divergence of the velocity field we can crank down the artificial viscosity.}
