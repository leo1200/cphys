...
\bluebox{\textbf{Box-Muller trick:} Given two uniformly distributed random numbers $u_1, u_2 \sim \mathcal{U}(0,1)$, we can generate two normally distributed random numbers $x, y \sim \mathcal{N}(0,1)$ by
\begin{equation}
...
\begin{itemize}
    ...
    of states $x_1,\dots,x_n$ in a single Markov chain under action of $f$ approaches $p_{eq}$ as the number of steps
    $n$ goes to infinity.
\end{itemize}
\greenbox{We therefore have two principal ways of sampling: \textbf{ensemble sampling} (we apply $f$ repeatedly
...
\begin{enumerate}
    ...
    \begin{equation}
        \begin{gathered}
            ...
            \text{e.g. } e \sim \mathcal{N}(0,\sigma^2) \text{ so }  q(x \rightarrow x') = \mathcal{N}(x,\sigma^2)
        \end{gathered}
    \end{equation}
    }
...
\begin{equation}
    \label{eq:post_int}
    \hat{I}_N = \frac{1}{N} \sum_{i=1}^{N} u(\vec{\mu}_i)
\end{equation}
}
